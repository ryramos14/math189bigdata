...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	sig = sigmoid(X @ W)
	addend = np.multiply(y, np.log(sig)) + np.multiply((1. - y), np.log(1. - sig))
	nll = (-sum(addend) + reg/2 * np.linalg.norm(W)**2).item(0)
...
def newton_step(X, y, W, reg=0.0):
	...
	diagonal = np.diag(np.squeeze(np.asarray(np.multiply(mu, 1. - mu))))
	H = X.T @ diagonal @ X + reg * np.eye(X.shape[1])
...
def get_description(X, y, W):
	...
	m, n = X.shape
	for i in range(m):
		actual, pred = y.item(index), pred.item(index)
		if actual == pred:
			a += 1
		if actual == 1:
			tot_r += 1
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	for i in range(len(reg_list)):
		...
		acc, precision, recall, f1 = get_description(X_test, y_test, W_opt)
	...
	"*** YOUR CODE HERE ***"
	reg_index = np.argmax(a_list)
	reg_opt = reg_list[reg_index]
